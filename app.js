...
function rolarExpressaoPersonalizada() {
  ...
  try {
    ...
    const textoResultado = `${userNick} rolou ${detalhe} = ${totalFinal}`;
...
function registrarRolagemHistorico(descricao, resultado, veioDoBroadcast = false) {
  ...
  item.innerHTML = `
    ...
    <strong style="color: #04d361; font-size: 1.2rem;">${textoRes}</strong>
  `;
...
// CRÔNICAS DE CAMELOT - APP.JS BLINDADO
// ==========================================
...
// Inicialização segura para não travar o script se o Supabase demorar
try {
  if (window.supabase) {
    supabaseClient = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
  }
